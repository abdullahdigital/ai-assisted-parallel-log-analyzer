...
impl ThreatDetector {
    ...
    fn check_brute_force(&mut self, log_entry: &LogEntry, rule: &Rule) -> Option<Alert> {
        if log_entry.event_type == "login_failed" {
            if let Some(user_id) = &log_entry.user_id {
                let key = (log_entry.ip_address.clone(), user_id.clone());
                let (attempts, last_attempt_time) = self.brute_force_attempts.entry(key.clone()).or_insert((0, log_entry.timestamp));

                if log_entry.timestamp - *last_attempt_time < Duration::seconds(rule.time_window_seconds as i64) {
                    *attempts += 1;
                } else {
                    *attempts = 1;
                }
                *last_attempt_time = log_entry.timestamp;
                ...
                if *attempts >= rule.threshold {
                    ...
                    return Some(Alert {
                        ...
                        description: format!("Brute-force attempt detected from IP {} for user {}", log_entry.ip_address, user_id),
    ...
    fn check_high_frequency_request(&mut self, log_entry: &LogEntry, rule: &Rule) -> Option<Alert> {
        let key = log_entry.ip_address.clone();
        let (count, last_request_time) = self.high_frequency_requests.entry(key.clone()).or_insert((0, log_entry.timestamp));

        if log_entry.timestamp - *last_request_time < Duration::seconds(rule.time_window_seconds as i64) {
            *count += 1;
        } else {
            *count = 1;
        }
        *last_request_time = log_entry.timestamp;
        ...
        if *count >= rule.threshold {
            ...
            return Some(Alert {
                ...
                description: format!("High-frequency requests detected from IP {}", log_entry.ip_address),
                log_entry_sample: Some(log_entry.clone()),
            });
        }
        None
    ...
    fn check_suspicious_ip_behavior(&mut self, log_entry: &LogEntry, rule: &Rule) -> Option<Alert> {
        ...
        if log_entry.timestamp - *last_event_time < Duration::seconds(rule.time_window_seconds as i64) {
        ...
        *last_event_time = log_entry.timestamp;
        ...
        if event_counts.len() >= rule.threshold {
            self.suspicious_ip_behavior.remove(&key);
            return Some(Alert {
                id: Uuid::new_v4().to_string(),
                timestamp: log_entry.timestamp,
                alert_type: AlertType::SuspiciousIpBehavior,
                description: format!("Suspicious IP behavior detected from IP {}: multiple event types", log_entry.ip_address),
                log_entry_sample: Some(log_entry.clone()),
            });
        }
        None
